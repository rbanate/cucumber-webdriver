...
defineSupportCode(({ Given }) => {
  ...
  Given(/^the element "([^"]*)?"( not)* matches the text "([^"]*)?"$/, checkEqualsText);

  Given(/^the element "([^"]*)?"( not)* contains the text "([^"]*)?"$/, checkContainsText);

  Given(/^the element "([^"]*)?"( not)* contains any text$/, checkContainsAnyText);
